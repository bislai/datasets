...
chaFavor = _.filter(mociones, function(res) { if (/CHA/.test(res.a_favor)) return res.fecha });
...
chaContra = _.filter(mociones, function(res) { if (/CHA/.test(res.en_contra)) return res.fecha });
...
chaAbstencion = _.filter(mociones, function(res) { if (/CHA/.test(res.abstencion)) return res.fecha });
chaAbstencion = _.countBy(chaAbstencion, function(res) { return (res.presentada) })

fs.writeFile('cha/cha-favor.json', JSON.stringify(chaFavor, null, 2), function(err) {
    if (err) {
        throw err;
    }
});
